...
SELECT NAME AS 국가명, CAPITAL AS 수도명, POPULATION AS 인구수, CONTINENT AS 대륙, AREA AS 면적
FROM tblCountry;
...
SELECT NAME AS 국가정보, CAPITAL AS 국가정보, POPULATION AS 국가정보
FROM TBLCOUNTRY
WHERE name = '대한민국';
...
SELECT FIRST_NAME||' '||LAST_NAME AS 이름, PHONE_NUMBER AS 연락처
FROM EMPLOYEES 
...
SELECT MANAGER_ID , FIRST_NAME||' '||LAST_NAME AS 이름, SALARY AS 급여, HIRE_DATE AS 고용날짜
FROM EMPLOYEES 
...
SELECT *
FROM TBLINSA 
WHERE CITY = '서울' AND (BASICPAY + SUDANG) >= 1500000;
...
SELECT *
FROM TBLINSA 
WHERE JIKWI IN ('사원', '대리') AND SUDANG <= 150000;
...
SELECT *
FROM TBLINSA 
WHERE CITY = '서울' AND JIKWI IN ('과장','부장') AND (BASICPAY*12) >= 20000000;
