...
-- 연계 회사 수정 / 조건 수정
-- UPDATE tblcompany SET <수정할 항목> = <수정내용> WHERE companynum = <회사번호>;												
UPDATE tblcompany SET companysize = '대기업' WHERE companynum = '44';

/*
UPDATE tblrequirement 
	SET score = <점수> , subjectnum = <과목번호>
...
SELECT * FROM tblitemchange WHERE itemchangenum = 24;	
...
FROM
	tblaccesscardreissue c
INNER JOIN vwstudent v	
		ON
	c.studentnum = v.studentnum
ORDER BY
	to_number(c.accesscardreissuenum);

-- 3. 재발급 신청이 들어온 교육생에 한해, 새로운 출입카드를 등록하고 배부한다.(update)
/*
UPDATE tblaccesscard
	SET isaccesscard = '1'
		WHERE accesscardnum = <출입카드번호>;
*/
UPDATE tblaccesscard
	SET isaccesscard = '1'
		WHERE accesscardnum = '14';
...
SELECT * FROM vwStudent WHERE studentname = '허수경';