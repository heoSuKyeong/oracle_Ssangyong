...
IS 
BEGIN
	INSERT INTO tblFail (failNum, studentNum, failDate, failReon) VALUES ((SELECT NVL(MAX(LPAD(failNum, 5, '0')), 0) + 1 FROM tblFail), pnum, pdate, preon);
EXCEPTION
	WHEN OTHERS THEN 
        DBMS_OUTPUT.PUT_LINE('등록에 실패했습니다.');
END procToFail;
...
BEGIN
    ...
    LOOP
        IF (flag = 0)
            THEN 
				DBMS_OUTPUT.PUT_LINE('───────────────────── 특정 상담일지 조회 ─────────────────────');
...
IS
BEGIN
	INSERT INTO tblInterviewer (interviewerNum,interviewerName,interviewerSsn,interviewerTel,interviewerDate,isPs)
...
IS
BEGIN
	IF pisPs = 0 OR pisPs = 1 THEN 
        UPDATE tblInterviewer SET isPs = pisPs WHERE interviewerNum = pnum;
...
IS
BEGIN
    INSERT INTO tblAssignment (signmentNum, subjectDetailNum, signmentContent)
    VALUES ((SELECT NVL(MAX(LPAD(signmentNum, 8, '0')), 0) + 1 FROM tblAssignment), pSubjectDetailNum, pAssignmentContent);
	EXCEPTION
	    WHEN OTHERS THEN
	        DBMS_OUTPUT.PUT_LINE('생성에 실패했습니다.');
...
BEGIN
    ...
    SET subjectDetailNum = pSubjectDetailNum, signmentContent = pAssignmentContent
    WHERE signmentNum = pAssignmentNum;
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('생성에 실패했습니다.');
...
IS
BEGIN
    DELETE FROM tblAssignment WHERE signmentNum = pAssignmentNum;
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('생성에 실패했습니다.');
...
CREATE OR REPLACE PROCEDURE procApplyChangeItem(
    ...
    pitemChangeReon VARCHAR,
    presult OUT VARCHAR2
)
IS
BEGIN
    INSERT INTO tblitemChange (itemChangeNum, itemDetailNum, itemChangeReon, ISReplacement, itemChangeDate)
			VALUES ((SELECT NVL(MAX(LPAD(itemChangeNum, 5, '0')), 0) + 1 FROM tblitemChange), pitemDetailNum, pitemChangeReon, 0, SYSDATE);
...
BEGIN
    ...
    procStudentExamScores(pStudentNum => 44); --학생번호 
END;
...
IS
BEGIN
    INSERT INTO tblAttENDanceApply (attENDanceApplyNum, studentNum, teacherNum, applyAttENDance, applyReon, applyDate)
                VALUES ((SELECT NVL(MAX(LPAD(attENDanceApplyNum, 5, '0')), 0) + 1 FROM tblAttENDanceApply), vStudentNum, vTeacherNum, vApplyAttENDance, vApplyReon, DEFAULT);
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    ...
    LOOP
        ...
        DBMS_OUTPUT.PUT_LINE('신청사유: ' || vRow.applyReon);
...
IS
BEGIN
    UPDATE tblAttENDanceApply SET applyAttENDance = pApplyAttENDance, applyReon = pApplyReon WHERE studentNum = pStudentNum AND TO_CHAR(applyDate, 'YYMMDD') = TO_CHAR(SYSDATE, 'YYMMDD');
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    ...
    INSERT INTO tblAccessCardReISsue (accessCardReISsueNum, studentNum, ISReISsue, reISsueReon)
			VALUES ((SELECT NVL(MAX(LPAD(accessCardReISsueNum, 5, '0')), 0) + 1 FROM tblAccessCardReISsue), pstudentNum, 0, preISsueReon);
...
DECLARE
	vresult VARCHAR2(300);
BEGIN
	procAddQuestion('37', '190', '겁나게 어려워부렁', vresult);
	DBMS_OUTPUT.PUT_LINE(vresult);
END;
...
DECLARE
	vresult VARCHAR2(300);
BEGIN
	procUpdateQuestion(1, '31','38', vresult);
	DBMS_OUTPUT.PUT_LINE(vresult);
END;
...
IS
BEGIN
    INSERT INTO tblAssignmentSubmit (signmentSubmitNum, signmentNum, studentNum, signmentExplain)
    VALUES ((SELECT NVL(MAX(LPAD(signmentSubmitNum, 8, '0')), 0) + 1 FROM tblAssignmentSubmit), pAssignmentNum, pStudentNum, pAssignmentExplain);
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('생성에 실패했습니다.');
...
BEGIN
    ...
    SELECT
        c.courseName AS 과정명,
        s.subjectName AS 과목명,
        a.signmentContent AS 과제명,
        sb.signmentSubmitNum AS 과제제출번호,
        st.studentName AS 교육생이름,
        sb.signmentExplain AS 과제풀이
    FROM tblAssignmentSubmit sb
        INNER JOIN tblAssignment a
            ON a.signmentNum = sb.signmentNum
                INNER JOIN tblSubjectDetail sd
    ...
    IN (
        ...
        FROM tblAssignmentSubmit sb2
            INNER JOIN tblAssignment a2
                ON a2.signmentNum = sb2.signmentNum
    ...
    ORDER BY c.courseName, s.subjectName, sb.studentNum, sb.signmentSubmitNum;
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('생성에 실패했습니다.');
...
DECLARE
    ...
    vSubjectName tblSubject.subjectName%TYPE;
    vAssignmentName tblAssignment.signmentContent%TYPE;
    vAssignmentSubmitNum tblAssignmentSubmit.signmentSubmitNum%TYPE;
    vStudentName tblInterviewer.interviewerName%TYPE;
    vAssignmentExplanation tblAssignmentSubmit.signmentExplain%TYPE;
BEGIN
...
BEGIN
    ...
    SET signmentNum = pAssignmentNum, studentNum = pStudentNum, signmentExplain = pAssignmentExplain
    WHERE signmentSubmitNum = pAssignmentSubmitNum;
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('생성에 실패했습니다.');
...
IS
BEGIN
    DELETE FROM tblAssignmentSubmit WHERE signmentSubmitNum = pAssignmentSubmitNum;
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('생성에 실패했습니다.');
